...
const resolveColor = (color: string) => {
  ...
  // If the variable points to an OKLCH value, we need to skip it or provide a 
  // hex/rgba fallback because standard Canvas gradients do not support oklch() 
  // strings in most stable browsers.
  if (!val || val.includes("oklch")) {
    // If theme is dark, use a darker grey, else a light grey
    const isDark = document.documentElement.classList.contains('dark') || 
                   document.documentElement.getAttribute('data-theme') === 'dark';
    return isDark ? "rgba(100, 116, 139, 0.4)" : "rgba(148, 163, 184, 0.3)";
  }
...
export const DotField = memo(function DotField({
  ...
  sparkle = false,
  waveAmplitude = 0,
  gradientFrom = "rgba(125, 211, 252, 0.2)",
  ...
  useEffect(() => {
    propsRef.current = {
      ...
      dotRadius,
      dotSpacing,
      gradientFrom: resolveColor(gradientFrom),
      gradientTo: resolveColor(gradientTo),
    };
    setResolvedGlowColor(resolveColor(glowColor));
    rebuildRef.current?.();
  }, [theme, gradientFrom, gradientTo, glowColor, dotRadius, dotSpacing]);
  ...
  useEffect(() => {
    ...
    function tick() {
      ...
      const crSq = cr * cr;
      const rad = Math.max(0.75, p.dotRadius);
      ...
      for (let i = 0; i < len; i++) {
        ...
        if (p.sparkle) {
          ...
          const drawRad = hash % 100 < 3 ? rad * 1.8 : rad;
          context.moveTo(drawX + drawRad, drawY);
          context.arc(drawX, drawY, drawRad, 0, TWO_PI);
        } else {
  ...
  return (
    <div className={`dot-field-container ${className}`} aria-hidden="true">
      ...
      <svg className="dot-field-glow" pointerEvents="none">
        <defs>
